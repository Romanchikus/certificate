...
class AddCertificateView(UpdateView):
    ...
    success_url = reverse_lazy("list_of_certificates")
    ...
    def get_object(self, queryset=None):
        if self.request.GET.get("check"):
            cert = get_object_or_404(
                Certificate, public_num=self.request.GET.get("public_num")
            )
        else:
...
class ListCertificateView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        kwargs["cert_form"] = PreCertificate(initial={"emitter": self.request.user})
        return context
